...
/**
 * O1 — expected score gain on the exam, weighted by each item's share of the syllabus.
 *
 * <pre>
 ...
 * without saturation, the marginal gain of a high-importance item never falls far enough for a
 ...
 * on an already-mastered item stop paying, which is what pushes the allocation to spread.
 *
 ...
 * material takes longer, so tau grows with the item's difficulty band. It lives in
...
public class ScoreGainObjective implements FitnessObjective {
    ...
    public double calculateReward(StudyPlan plan, EvolutionContext context) {
        ...
        for (int i = 0; i < genes; i++) {
            PlanningItem item = ordem.item(i);
            double importance = vetores.normalizedImportance(i);
            if (importance == 0.0) {
                log.warn("The planning item '{}' does not have an importance score.", item.name());
            }
            score += importance * LearningModel.mastery(item.difficultyBand(), plan.daysAt(ordem, i));
        }
        return score;
    }
